...
TEST_CASE("EllCalc, test deep cut") {
    ...
    CutStatus status;
    std::tuple<double, double, double> result;
    std::tie(status, result) = ell_calc.calc_deep_cut(0.11, 0.01);
    CHECK_EQ(status, CutStatus::NoSoln);
    std::tie(status, result) = ell_calc.calc_deep_cut(0.01, 0.01);
    CHECK_EQ(status, CutStatus::Success);

    std::tie(status, result) = ell_calc.calc_deep_cut(0.05, 0.01);
    CHECK_EQ(status, CutStatus::Success);
    std::tie(rho, sigma, delta) = result;
...
TEST_CASE("EllCalc, test parallel deep cut") {
    ...
    CutStatus status;
    std::tuple<double, double, double> result;
    std::tie(status, result) = ell_calc.calc_parallel_deep_cut(0.07, 0.05, 0.01);
    CHECK_EQ(status, CutStatus::NoSoln);

    std::tie(status, result) = ell_calc.calc_parallel_deep_cut(0.0, 0.05, 0.01);
    CHECK_EQ(status, CutStatus::Success);
    std::tie(rho, sigma, delta) = result;
    CHECK_EQ(rho, doctest::Approx(0.02));
    CHECK_EQ(sigma, doctest::Approx(0.8));
    ...
    std::tie(status, result) = ell_calc.calc_parallel_deep_cut(0.05, 0.11, 0.01);
    CHECK_EQ(status, CutStatus::Success);
    std::tie(rho, sigma, delta) = result;
    CHECK_EQ(rho, doctest::Approx(0.06));
    CHECK_EQ(sigma, doctest::Approx(0.8));
    ...
    std::tie(status, result) = ell_calc.calc_parallel_deep_cut(0.01, 0.04, 0.01);
    CHECK_EQ(status, CutStatus::Success);
    std::tie(rho, sigma, delta) = result;
...
//     CutStatus status;
//     std::tuple<double, double, double> result;
//     std::tie(status, result) = ell_calc.calc_parallel_deep_cut_q(-0.04, 0.0625, 0.01);
...
TEST_CASE("EllCalc, test deep cut q") {
    ...
    CutStatus status;
    std::tuple<double, double, double> result;
    std::tie(status, result) = ell_calc.calc_deep_cut_q(0.11, 0.01);
    CHECK_EQ(status, CutStatus::NoSoln);
    std::tie(status, result) = ell_calc.calc_deep_cut_q(0.01, 0.01);
    CHECK_EQ(status, CutStatus::Success);
    std::tie(status, result) = ell_calc.calc_deep_cut_q(-0.05, 0.01);
    CHECK_EQ(status, CutStatus::NoEffect);
...
TEST_CASE("EllCalc, test parallel deep cut q") {
    ...
    CutStatus status;
    std::tuple<double, double, double> result;
    std::tie(status, result) = ell_calc.calc_parallel_deep_cut_q(0.07, 0.03, 0.01);
    CHECK_EQ(status, CutStatus::NoSoln);
    std::tie(status, result) = ell_calc.calc_parallel_deep_cut_q(-0.04, 0.0625, 0.01);
    CHECK_EQ(status, CutStatus::NoEffect);
