...
TEST_CASE("EllCalcCore, test central cut") {
    ...
    auto [rho, sigma, delta] = ell_calc_core.calc_central_cut(0.1);
...
TEST_CASE("EllCalcCore, test bias cut") {
    auto ell_calc_core = EllCalcCore(4);
    auto [rho, sigma, delta] = ell_calc_core.calc_bias_cut(0.05, 0.1);
...
TEST_CASE("EllCalcCore, test parallel cut") {
    auto ell_calc_core = EllCalcCore(4);
    auto [rho, sigma, delta] = ell_calc_core.calc_parallel_cut(0.01, 0.04, 0.01);
...
TEST_CASE("EllCalcCore, test parallel cut (no effect)") {
    auto ell_calc_core = EllCalcCore(4);
    auto [rho, sigma, delta] = ell_calc_core.calc_parallel_cut(-0.04, 0.0625, 0.01);
