...
inline Arr view(const Arr& a, const Range& rows, const Range& cols) {
    ...
    for (size_t i = 0; i < out_r; ++i)
        for (size_t j = 0; j < out_c; ++j)
            out(i, j) = a(rs + i * rstep, cs + j * cstep);
    return out;
}
...
// Element-wise math
// ---------------------------------------------------------------------------
inline Arr cos(const Arr& a) { Arr o = make_same_shape(a); for (size_t i = 0; i < a.size(); ++i) o(i) = std::cos(a(i)); return o; }
inline Arr log(const Arr& a) { Arr o = make_same_shape(a); for (size_t i = 0; i < a.size(); ++i) o(i) = std::log(a(i)); return o; }
inline Arr abs(const Arr& a) { Arr o = make_same_shape(a); for (size_t i = 0; i < a.size(); ++i) o(i) = std::abs(a(i)); return o; }
inline Arr exp(const Arr& a) { Arr o = make_same_shape(a); for (size_t i = 0; i < a.size(); ++i) o(i) = std::exp(a(i)); return o; }
inline Arr sqrt(const Arr& a) { Arr o = make_same_shape(a); for (size_t i = 0; i < a.size(); ++i) o(i) = std::sqrt(a(i)); return o; }
...
// ---------------------------------------------------------------------------
inline Arr operator-(const Arr& a) { Arr o = make_same_shape(a); for (size_t i = 0; i < a.size(); ++i) o(i) = -a(i); return o; }
inline Arr operator+(const Arr& a, const Arr& b) { assert(a.size() == b.size()); Arr o = make_same_shape(a); for (size_t i = 0; i < a.size(); ++i) o(i) = a(i) + b(i); return o; }
inline Arr operator-(const Arr& a, const Arr& b) { assert(a.size() == b.size()); Arr o = make_same_shape(a); for (size_t i = 0; i < a.size(); ++i) o(i) = a(i) - b(i); return o; }
inline Arr operator*(const Arr& a, const Arr& b) { assert(a.size() == b.size()); Arr o = make_same_shape(a); for (size_t i = 0; i < a.size(); ++i) o(i) = a(i) * b(i); return o; }
...
inline Arr operator<=(const Arr& a, double s) { Arr o = make_same_shape(a); for (size_t i = 0; i < a.size(); ++i) o(i) = (a(i) <= s) ? 1.0 : 0.0; return o; }
inline Arr operator>=(const Arr& a, double s) { Arr o = make_same_shape(a); for (size_t i = 0; i < a.size(); ++i) o(i) = (a(i) >= s) ? 1.0 : 0.0; return o; }
inline Arr operator<(const Arr& a, double s)  { Arr o = make_same_shape(a); for (size_t i = 0; i < a.size(); ++i) o(i) = (a(i) < s) ? 1.0 : 0.0; return o; }
inline Arr operator>(const Arr& a, double s)  { Arr o = make_same_shape(a); for (size_t i = 0; i < a.size(); ++i) o(i) = (a(i) > s) ? 1.0 : 0.0; return o; }
