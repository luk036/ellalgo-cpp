...
template <typename T>
auto EllCore::update_cc(Vec &grad, const T &beta) -> CutStatus {
  // const auto& [grad, beta] = cut;
  // auto Qg = zeros({this->_n}); // initial x0
  std::valarray<double> Qg(0.0, this->_n);
  auto omega = 0.0;
  ...
  this->_helper._tsq = this->_kappa * omega;
  auto status = this->_update_cut_cc(beta);
  if (status != CutStatus::Success) {
    return status;
  }
...
template <typename T>
auto EllCore::update_stable_cc(Vec &g, const T &beta) -> CutStatus {
  // calculate inv(L)*grad: (n-1)*n/2 multiplications
  auto invLg{g}; // initially
  for (auto i = 1U; i != this->_n; ++i) {
    for (auto j = 0U; j != i; ++j) {
...
// Instantiation
template CutStatus EllCore::update_cc(Vec &grad, const double &beta);

template CutStatus EllCore::update_cc(Vec &grad, const Vec &beta);

template CutStatus EllCore::update_stable_cc(Vec &grad, const double &beta);

template CutStatus EllCore::update_stable_cc(Vec &grad, const Vec &beta);
